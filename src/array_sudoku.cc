...
void test_is_grid_valid(std::vector<std::vector<int>> grid, bool expected) {
  std::cout << std::endl << "Looking to see if this sudoku grid is valid:" << std::endl;
  for (const auto& row : grid) {
    print_vector(row);
  }

  const auto actual = is_grid_valid(grid);
